...
regd_users.post("/register", (req,res) => {
    ...
    if(username&&password){
        ...
        if(present.length===0){
            ...
            return res.status(201).json({message:"Customer successfully registed.Now you can login"})
        }
        else{
...
regd_users.delete("/auth/review/:isbn", (req, res) => {
    let ISBN = req.params.isbn;
    books[ISBN].reviews = {}
    return res.send(`Reviews for the ISBN ${req.params.isbn} posted by the user test deleted`);
});
